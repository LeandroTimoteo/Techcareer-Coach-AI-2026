...
import { LayoutDashboard, FileText, Linkedin, MessageSquare, Users, Sparkles, Globe } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    generateCareerAdvice(t.tipPrompt, 'general', language)
      .then((res) => {
        console.log("Resposta bruta da API Gemini:", res); // LOG para validar chave
        setDailyTip(res);
      })
      .catch((err) => {
        console.error("Erro na API Gemini:", err);
        setDailyTip(language === 'pt' ? 'Sempre continue aprendendo!' : 'Always keep learning!');
      });
  }, [language]);
            ...
            <div className="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-2xl p-8 text-white shadow-lg">
              ...
              {dailyTip && (
                <div className="bg-white/10 p-4 rounded-lg border border-white/20 italic">
                  <MarkdownRenderer content={dailyTip} />
                </div>
              )}
            </div>
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col lg:flex-row">
          ...
          {tools.map(tool => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-2 rounded-lg ${
                activeTool === tool.id ? 'bg-blue-50 text-blue-700 font-bold' : 'text-slate-600 hover:bg-slate-50'
              }`}
            >
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        <header className="h-16 bg-white border-b px-8 flex items-center justify-between shrink-0">
          <h2 className="font-bold text-slate-700 uppercase tracking-widest text-sm">{activeTool}</h2>
          <button
